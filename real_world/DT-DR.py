...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "DT-DR"
    mf = MF_DT_DR(num_user, num_item, embedding_k=model_args['embedding_k'],batch_size=train_args['batch_size'], embedding_k1=model_args['embedding_k1'])
    mf.cuda()
    mf.fit(x_train, y_train, 
        lr=model_args['lr'], alpha = train_args['alpha'], beta = train_args['beta'], theta = train_args['theta'],
        lamb_prop=model_args['lamb_prop'],
        lamb_pred=model_args['lamb_pred'],
        lamb_imp=model_args['lamb_imp'], G = train_args["G"],
    ...
    print("***"*5 + "[DT-DR]" + "***"*5)
    print("[DT-DR] test mse:", mse_mf)
    print("[DT-DR] test mse:", mae_mf)
    print("[DT-DR] test auc:", auc)
    print("[DT-DR] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[DT-DR] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[DT-DR] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":128, "gamma": 0.05, "G":4, "alpha": 0.5, "beta": 0.1, "theta":1}
        args.model_args = {"embedding_k":8, "embedding_k1": 8, "lr":0.01, "lamb_prop": 1e-3,"lamb_pred": 1e-4, "lamb_imp": 1e-4}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":4096, "gamma": 0.05, "G":4, "alpha": 0.5, "beta": 1e-5, "theta":1}
        args.model_args = {"embedding_k":16, "embedding_k1": 16, "lr":0.01, "lamb_prop": 1e-3,"lamb_pred": 1e-5, "lamb_imp": 1e-1}
    elif args.dataset=="kuai":
        args.train_args = {"batch_size":4096, "gamma": 0.05, "G":4, "alpha": 0.5, "beta": 1e-5, "theta":1}
        args.model_args = {"embedding_k":16, "embedding_k1": 16, "lr":0.01, "lamb_prop": 1e-2,"lamb_pred": 1e-5, "lamb_imp": 1e-2}
    return args
