...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "ESMM"
    mf = MF_ESMM(num_user, num_item, embedding_k=model_args['embedding_k'],batch_size_prop = train_args['batch_size_prop'])
    mf.cuda()
    ...
    print("***"*5 + "[ESMM]" + "***"*5)
    print("[ESMM] test mse:", mse_mf)
    print("[ESMM] test mse:", mae_mf)
    print("[ESMM] test auc:", auc)
    print("[ESMM] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[ESMM] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[ESMM] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size_prop":1024, "gamma": 0.05, 'alpha': 2}
        args.model_args = {"embedding_k":8, "lr":0.01, "lamb_prop": 1e-3,"lamb_pred": 1e-5}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size_prop":32764, "gamma": 0.05, 'alpha': 1}
        args.model_args = {"embedding_k":32, "lr":0.005, "lamb_prop": 1e-6,"lamb_pred": 1e-6}
