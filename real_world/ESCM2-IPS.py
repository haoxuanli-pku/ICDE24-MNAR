...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "ESCM2-IPS"
    mf = MF_ESCM2_IPS(num_user, num_item, batch_size=train_args['batch_size'],  embedding_k=model_args['embedding_k'])
    mf.cuda()
    mf.fit(x_train, y_train, 
        lr=model_args['lr'],
        alpha = train_args['alpha'], beta = train_args['beta'], theta = train_args['theta'],
    ...
    print("***"*5 + "[ESCM2-IPS]" + "***"*5)
    print("[ESCM2-IPS] test mse:", mse_mf)
    print("[ESCM2-IPS] test mse:", mae_mf)
    print("[ESCM2-IPS] test auc:", auc)
    print("[ESCM2-IPS] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[ESCM2-IPS] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[ESCM2-IPS] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":128, "batch_size_prop":128, "gamma": 0.05, "alpha": 1, "beta": 1, "theta": 1e-3}
        args.model_args = {"embedding_k":4, "lr":0.01, "lamb_pred": 1e-4}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":4096, "batch_size_prop":4096, "gamma": 0.05, "alpha": 1, "beta": 1, "theta": 1e-4}
        args.model_args = {"embedding_k":32, "lr":0.005, "lamb_pred": 1e-5}
