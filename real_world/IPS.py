...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "IPS"
    mf = MF_IPS(num_user, num_item, batch_size=train_args['batch_size'], batch_size_prop = train_args['batch_size_prop'], embedding_k=model_args['embedding_k'])
    mf.cuda()
    mf._compute_IPS(x_train, lr = model_args['lr'], lamb = model_args['lamb_prop'])
    ...
    print("***"*5 + "[IPS]" + "***"*5)
    print("[IPS] test mse:", mse_mf)
    print("[IPS] test mse:", mae_mf)
    print("[IPS] test auc:", auc)
    print("[IPS] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[IPS] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[IPS] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        ...
        args.model_args = {"embedding_k":16, "lr":0.01, "lamb": 1e-3, "lamb_prop": 1e-6}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":4096, "batch_size_prop":32764, "gamma": 0.05}
        args.model_args = {"embedding_k":32, "lr":0.01, "lamb": 1e-4, "lamb_prop": 1e-4}
