...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "DT-IPS"
    mf = MF_DT_IPS(num_user, num_item, embedding_k=model_args['embedding_k'], embedding_k1=model_args['embedding_k1'], batch_size=train_args['batch_size'])
    mf.cuda()
    mf.fit(x_train, y_train, alpha = train_args['alpha'], beta = train_args['beta'],
        lr=model_args['lr'], 
    ...
    print("***"*5 + "[DT-IPS]" + "***"*5)
    print("[DT-IPS] test mse:", mse_mf)
    print("[DT-IPS] test mse:", mae_mf)
    print("[DT-IPS] test auc:", auc)
    print("[DT-IPS] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[DT-IPS] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[DT-IPS] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":128, "gamma": 0.05, "G":6, "alpha": 0.5, "beta": 1e-2}
        args.model_args = {"embedding_k":8, "embedding_k1": 8, "lr":0.01, "lamb_prop": 1e-2,"lamb_pred": 1e-4}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":4096, "gamma": 0.05, "G":4, "alpha": 0.5, "beta": 1e-3}
        args.model_args = {"embedding_k":16, "embedding_k1": 16, "lr":0.01, "lamb_prop": 1e-4,"lamb_pred": 1e-5}
    elif args.dataset=="kuai":
        args.train_args = {"batch_size":4096, "gamma": 0.05, "G":4, "alpha": 0.5, "beta": 1e-3}
        args.model_args = {"embedding_k":16, "embedding_k1": 16, "lr":0.01, "lamb_prop": 1e-3,"lamb_pred": 1e-4}
    return args
