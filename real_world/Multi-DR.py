...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "Multi-DR"
    mf = MF_Multi_DR(num_user, num_item, embedding_k=model_args['embedding_k'],batch_size_prop = train_args['batch_size_prop'])
    mf.cuda()
    ...
    print("***"*5 + "[Multi-DR]" + "***"*5)
    print("[Multi-DR] test mse:", mse_mf)
    print("[Multi-DR] test mse:", mae_mf)
    print("[Multi-DR] test auc:", auc)
    print("[Multi-DR] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[Multi-DR] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[Multi-DR] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size_prop":128, "gamma": 0.05}
        args.model_args = {"embedding_k":8, "lr":0.01, "lamb_prop": 1e-4,"lamb_pred": 1e-4,"lamb_imp": 1e-2}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size_prop":4096, "gamma": 0.05}
        args.model_args = {"embedding_k":16, "lr":0.05, "lamb_prop": 1e-5,"lamb_pred": 1e-6,"lamb_imp": 1e-4}
    elif args.dataset=="kuai":
        args.train_args = {"batch_size_prop":4096, "gamma": 0.05}
        args.model_args = {"embedding_k":16, "lr":0.01, "lamb_prop": 1e-4,"lamb_pred": 1e-4,"lamb_imp": 1e-2}
    return args
