...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "DR-MSE"
    mf = MF_DR_MSE(num_user, num_item)
    mf.cuda()
    ...
    print("***"*5 + "[DR-MSE]" + "***"*5)
    print("[DR-MSE] test mse:", mse_mf)
    print("[DR-MSE] test mse:", mae_mf)
    print("[DR-MSE] test auc:", auc)
    print("[DR-MSE] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[DR-MSE] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[DR-MSE] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":128, 'gamma':0.5}
        args.model_args = {"lr":0.05, "lamb": 1e-3}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":2048, 'gamma':0.5}
        args.model_args = {"lr":0.05, "lamb": 1e-4}
    elif args.dataset=="kuai":
        args.train_args = {"batch_size":2048, 'gamma':0.25}
        args.model_args = {"lr":0.05, "lamb": 1e-4}
    return args
