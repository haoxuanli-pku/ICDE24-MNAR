...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "TDR"
    mf = MF_TDR(num_user, num_item, batch_size = train_args['batch_size'],
    ...
    print("***"*5 + "[TDR]" + "***"*5)
    print("[TDR] test mse:", mse_mf)
    print("[TDR] test mse:", mae_mf)
    print("[TDR] test auc:", auc)
    print("[TDR] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[TDR] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[TDR] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":128, 'gamma':0.1, 'G':3}
        args.model_args = {"lr":0.05, "lamb": 1e-4, "embedding_k":4}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":2048, 'gamma':0.15, 'G':3}
        args.model_args = {"lr":0.05, "lamb": 1e-5, "embedding_k":4}
    elif args.dataset=="kuai":
        args.train_args = {"batch_size":2048, 'gamma':0.1, 'G':1}
