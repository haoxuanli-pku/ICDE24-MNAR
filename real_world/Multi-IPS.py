...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "Multi-IPS"
    mf = MF_Multi_IPS(num_user, num_item, batch_size=train_args['batch_size'], embedding_k=model_args['embedding_k'],batch_size_prop = train_args['batch_size_prop'])
    mf.cuda()
    ...
    print("***"*5 + "[Multi-IPS]" + "***"*5)
    print("[Multi-IPS] test mse:", mse_mf)
    print("[Multi-IPS] test mse:", mae_mf)
    print("[Multi-IPS] test auc:", auc)
    print("[Multi-IPS] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[Multi-IPS] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[Multi-IPS] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        ...
        args.model_args = {"embedding_k":256, "lr":0.005, "lamb": 1e-2}
