...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    print("***"*5 + "[MRDR-JL]" + "***"*5)
    print("[MRDR-JL] test mse:", mse_mf)
    print("[MRDR-JL] test mse:", mae_mf)
    print("[MRDR-JL] test auc:", auc)
    print("[MRDR-JL] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[MRDR-JL] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[MRDR-JL] {}:{:.6f}".format(
