...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "CVIB"
    mf = MF_CVIB(num_user, num_item, embedding_k=model_args['embedding_k'])
    mf.cuda()
    ...
    print("***"*5 + "[CVIB]" + "***"*5)
    print("[CVIB] test mse:", mse_mf)
    print("[CVIB] test mse:", mae_mf)
    print("[CVIB] test auc:", auc)
    print("[CVIB] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[CVIB] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[CVIB] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":128, "gamma": 0.05}
        args.model_args = {"embedding_k":16, "lr":0.01, "lamb": 1e-4}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":1024, "gamma": 0.05}
        args.model_args = {"embedding_k":32, "lr":0.01, "lamb": 1e-4}
    elif args.dataset=="kuai": 
        args.train_args = {"batch_size":1024, "gamma": 0.05}
        args.model_args = {"embedding_k":16, "lr":0.05, "lamb": 1e-4}
    return args
