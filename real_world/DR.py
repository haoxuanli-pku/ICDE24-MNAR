...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "DR"
    mf = MF_DR(num_user, num_item, batch_size=train_args['batch_size'], embedding_k=model_args['embedding_k'],batch_size_prop = train_args['batch_size_prop'])
    mf.cuda()
    mf._compute_IPS(x_train, lr = model_args['lr'], lamb=model_args['lamb'] )
    ...
    print("***"*5 + "[DR]" + "***"*5)
    print("[DR] test mse:", mse_mf)
    print("[DR] test mse:", mae_mf)
    print("[DR] test auc:", auc)
    print("[DR] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[DR] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[DR] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        ...
        args.model_args = {"embedding_k":16, "lr":0.01, "lamb": 1e-3}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":4096, "batch_size_prop":32764, "gamma": 0.05}
        args.model_args = {"embedding_k":32, "lr":0.005, "lamb": 1e-5}
