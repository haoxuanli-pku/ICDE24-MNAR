...
def train_and_eval(dataset_name, train_args, model_args):
    ...
    "DIB"
    mf = MF_DIB(num_user, num_item,batch_size=train_args['batch_size'])
    mf.cuda()
    ...
    print("***"*5 + "[DIB]" + "***"*5)
    print("[DIB] test mse:", mse_mf)
    print("[DIB] test mse:", mae_mf)
    print("[DIB] test auc:", auc)
    print("[DIB] {}:{:.6f}".format(
            top_k_names[2].replace("_", "@"), np.mean(ndcgs[top_k_names[2]])))
    print("[DIB] {}:{:.6f}".format(top_k_names[3].replace("_", "@"), f1))
    print("[DIB] {}:{:.6f}".format(
...
def para(args):
    if args.dataset=="coat":
        args.train_args = {"batch_size":256, "gamma": 1, "alpha": 1}
        args.model_args = {"lr":0.01, "lamb": 1e-3}
    elif args.dataset=="yahoo":
        args.train_args = {"batch_size":2048, "gamma": 0.5, "alpha": 0.5}
        args.model_args = {"lr":0.01, "lamb": 1e-4}
    elif args.dataset=="kuai":
        args.train_args = {"batch_size":2048, "gamma": 1, "alpha": 0.1}
